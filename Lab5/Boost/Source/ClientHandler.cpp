...
void ClientHandler::handleModifyCommand() {
    ...
    if (command == 1) {
        ...
        if (!pipeClient->sendRequest(commitRequest)) ExceptionHandler::printError("Failed to send commit request");
        else {
            PipeResponse commitResponse;
            if (!pipeClient->receiveResponse(commitResponse)) ExceptionHandler::printError("Failed to receive commit response");
            else if (commitResponse.status != PIPE_RESPONSE_OK) std::cout << "Commit failed" << std::endl;
            else std::cout << "Record updated" << std::endl;
        }
    }
    else {
        std::cout << "Changes were not sent" << std::endl;
    }
