...
go_bandit([]{
    describe("Array", []{
        describe("constructor", []{
            ...
            it("constructor with size 3 creates array", [&]{
                (*a3)[0];
            });
            ...
            it("constructor with size 2 initializes first element", [&]{
                AssertThat((*a2)[0], Equals(0));
            });

            it("constructor with size 2 initializes second element", [&]{
                AssertThat((*a2)[1], Equals(0));
            });

            it("constructor with size 3 initializes all elements to zero", [&]{
                AssertThat((*a3)[2], Equals(0));
            });
        });
        ...
        describe("printArray", []{
            ...
            it("ends output with newline character", [&]{
                ...
                a1->printArray();
                std::string output = out.oss.str();
                char lastChar = output.empty() ? '\0' : output[output.length() - 1];
                AssertThat(lastChar, Equals('\n'));
            });
        });
        ...
        describe("checkArrayInitialization", []{
            ...
            it("does not throw on array with size 1", [&]{
                a1->checkArrayInitialization();
            });

            it("does not throw on array with size 5", [&]{
                a5->checkArrayInitialization();
            });
        });
